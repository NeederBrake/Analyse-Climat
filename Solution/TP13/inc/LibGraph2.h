 ...
 *  Créez un nouveau projet C++ Visual Studio de type \b Projet \b vide pour la plateforme Windows :
 *  \image html create-project.png
 *  \subsection filecopy Copie des fichiers
 *  \li Copier les répertoires \c lib (qui se trouve dans le sous répertoire \c v142 ) et \c inc dans le répertoire de votre projet (à côté du fichier \c *.vcxproj).
 *  \li Copier les fichiers \c lib\\Release\\LibGraph2.dll et \c lib\\Debug\\LibGraph2.dll respectivement dans les répertoires \c x64\\Release et \c x64\\Debug  de votre solution :
 *  \note Les répertoires \c x64\\Debug et \c x64\\Release sont les répertoires où votre programme exécutable sera créé. Ces répertoires sont
 *  créés lors de la compilation. Comme vous n'avez pas encore compilé votre projet, ils n'existent probablement pas. Vous devez donc les créer vous-même.
 *
 *  \image html install-1.png "Organisation des fichiers"
 *  \subsection projectsetup Configuration du projet
 *  \li Ouvrir le gestionnaire de configuration (menu \b Générer / \b Gestionnaire \b de \b configurations...) puis dans la liste \b Plateforme \b de \b la \b solution \b active, choisissez <Modifier...> puis supprimez la plateforme x86 :
 *  \image html projet-configurations.png "Gestionnaire de configuration"
 *  \li Ouvrir les propriétés du projet (Dans Visual Studio, faites un clic droit sur le projet puis Propriétés)
 *  \li Choisissez \b "Toutes les configurations" dans la liste déroulante \b "Configuration :" puis dans la page \b "C/C++" / \b "Répertoire VC++", ajoutez \c ";$(ProjectDir)inc" dans le champ \b "Répertoires Include" et \c \c ";$(ProjectDir)lib\$(Configuration)" dans le champs \b "Répertoires de bibliothèques" :
 ...
 *  \li Dans la page \b "Editeur de liens" / \b "Système", définissez le \c Sous-système comme \b "Windows (/SUBSYSTEM:WINDOWS)" :
 *  \image html subsystem-windows.png
 *  \li Validez. --> LibGraph2 est maintenant configuré pour votre projet. Choisissez maintenant un niveau d'utilisation pour un exemple de programme principal.
 * \else
 *  \section install Installation
...
namespace LibGraph2
{
  ...
  public:
     ...
     *
     * Cet opérateur convertit la chaîne CString en une chaine std::string. Cela permet d'utiliser
     * la classe CString en lieu et place d'une chaine std::string.
     *
     * \see
     * Classe : CString
     */
...
#define ILibGraph2 ILibGraph2_Exp
  ...
  public:
     ...
     * 
     * Demande à rafraichir la fenêtre. L'événement evt_type::evtRefresh sera envoyé dès que possible après l'appel de cette fonction.
     * 
     * \see
     * Classe : \if NIVEAU3 ILibGraph2_Adv \endif \if NIVEAU4 ILibGraph2_Exp \endif \n
     ...
     * \ingroup DrawingProperties
     */
    virtual void setPen(ARGB color, float fWidth, pen_DashStyles style = pen_DashStyles::Solid) = 0;
    /*!
     ...
     * \see
     * Classes : CString, \if NIVEAU3 ILibGraph2_Adv \endif \if NIVEAU4 ILibGraph2_Exp \endif \n
     * Types de données : msgbtn_types, msgicon_types, msgdefbtn_vals, msgbtn_answer
     * \ingroup DialogManagement
     */
    virtual msgbtn_answer guiMessageBox(
      const CString& strTitle = L"", 
      const CString& strText = L"", 
...
//Fonctions communes aux niveaux Novice, Débutant et Intermédiaire
   ...
   * \ingroup DrawingProperties
   */
  LIBGRAPH2_API void setPen(ARGB color, float fWidth, pen_DashStyles style = pen_DashStyles::Solid);
  /*!
   * \brief Définit le style du pinceau par une couleur pleine.
   * 
   * Permet de définir la couleur de remplissage des formes fermées.
...
//Fonctions communes aux niveaux Débutant et Intermédiaire
   ...
   *     {
   *       //Clauses "case" pour traiter les événements
   *       //Si l'événement est un événement clavier, on peut connaitre la touche à l'aide de la variable evt_type::evtKeyCode
   *       //Si l'événement est un événement souris, on peut connaitre la position de la souris à l'aide des variables evt_type::evtMouseX et evt_type::evtMouseY
   *     }
   *   }
   * \endcode
   *
   ...
   * \ingroup DialogManagement
   */
  LIBGRAPH2_API msgbtn_answer guiMessageBox(
    const char* strTitle = "",
    const char* strText = "",
    msgbtn_types btns = msgbtn_types::MsgBtnOK,
    msgicon_types icon = msgicon_types::MsgIcnNone,
